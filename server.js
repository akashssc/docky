...
// Middleware
app.use(cors({
    origin: 'https://dockkyy.onrender.com',
    ...
    bcrypt.hash(password, 10, (err, hash) => {
        ...
        db.run('INSERT INTO users (username, email, password) VALUES (?, ?, ?)', [username, email, hash], function(err) {
            if (err) {
                ...
                return res.status(400).json({ error: 'Registration error: ' + err.message });
            }
...
app.post('/upload', upload.single('file'), (req, res) => {
    ...
    db.get(query, [value], (err, user) => {
        ...
        bcrypt.compare(password, user.password, (err, result) => {
            ...
            // User authenticated, save file
            db.run('INSERT INTO files (user_id, filename, originalname) VALUES (?, ?, ?)', 
                [user.id, file.filename, file.originalname], function(err) {
                if (err) return res.status(500).json({ error: 'DB error' });
